  ...
  bkms-cli app appspec underlay-ip enable --app my-app --env prod`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			if err := appspec.SetEnabledHandler(
				...
				return errors.Wrap(err, "enable underlay-ip")
			}

			if envName == "" {
				fmt.Printf("Successfully enabled underlay-ip in default config for app %s\n", appID)
			} else {
				fmt.Printf("Successfully enabled underlay-ip for app %s in env %s\n", appID, envName)
			}
			return nil
		},
	}
