  ...
  bkms-cli app appspec underlay-ip reset --app my-app --env prod`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err := appspec.ResetHandler(
				cmd.Context(), appID, envName, client.AppSpecSectionTkeRouteEni,
			); err != nil {
				return errors.Wrap(err, "reset underlay-ip")
			}

			fmt.Printf("Successfully reset underlay-ip for app %s in env %s to default\n", appID, envName)
			return nil
		},
	}
